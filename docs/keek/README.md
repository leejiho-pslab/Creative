...
> 📍 **이 사본의 출처**: 원본은 `leejiho-pslab/pslab` 저장소의
> `claude/keek-product-instagram-analysis-i9onxa` 브랜치 `docs/keek/` 에 있다.
> 2026-07-27 2차 검증과 유튜브 분석까지 반영한 시점의 사본이다.
> 문서 안의 워크플로 경로(`.github/workflows/keek-product-ig-pull.yml` 등)는 **pslab 저장소 기준**이다.
...
| [`youtube.md`](./youtube.md) | 🆕 **유튜브 채널 분석 (@keek1726, 28편 전량)** — **척추 전문의 검증 자산**·강철부대W 22만 조회·**AIR TECH 재정의** |
...
| [`HANDOFF.md`](./HANDOFF.md) | 세션 인수인계 — 접근 확인 절차·확보된 것·다음 작업 |
| [`verification-2026-07-27b.md`](./verification-2026-07-27b.md) | 🔄 **2차 검증 (스토어프론트 직접 조회)** — **공식 사이즈표 확보** · 재고 독립 검증 · **사실관계 2건 정정** |

> ⚠️ **2차 검증에서 정정된 사항이 있다.** 제품 문서를 읽기 전에 `verification-2026-07-27b.md` §7 요약표를 볼 것.
> 특히 **블로거 실측 사이즈는 폐기**됐고(공식표와 불일치), **1793 "계속 팔리고 있다"는 사실이 아니다**(품절 처리됨).

## 수집 경로

초판 수집 당시 실행 환경의 egress 정책이 아래 호스트를 차단했다(`CONNECT 403`).
...
> **2026-07-27 2차 검증 시점의 접근 상태 변화**: `keek-line.com`·`cafe24img.poxo.com`·`graph.facebook.com`이 **열렸고**,
> `api.cafe24.com`은 **차단 유지**다. 자사몰이 열리면 Admin API 없이도 **상세 HTML에 심긴 옵션 JSON에서 변형별 재고**를,
> **별도 이미지 파일에서 공식 사이즈표**를 얻을 수 있다 — API가 막혔을 때의 대체 경로다.
> 단 인스타그램은 `graph.facebook.com`이 열려도 **토큰이 없으면 무의미**하므로 워크플로 경유가 유일하다.
